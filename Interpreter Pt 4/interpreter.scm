...
; interpret-classes recursively evaluates all class definitions in the statement list.
...
; Mclass-state adds a class from the given statement to the class environment
; the environment will have the class and their definitions which include fields and functions/closures.
(define Mclass-state
...
; Mstate-assignment-dot handles variable assignment for class variables.
...
; Definitions for Mstate-assignment-dot
(define assignment-dot-env cadadr)
(define assignment-dot-var (lambda (v) (caddr (cadr v))))
(define assignment-dot-value caddr)
  
; Mstate-begin should be called whenever entering a block of code with curly braces
...
; Mstate-static-func handles static function declarations.
...
; Mstate-funcall is used when a function is called without the calling line needing 
; its return value, executes the function and then returns the environment.
; Statement format:
...
; Mstate-tcf modifies the state based on a try-catch-finally block.
...
; catch-begin is the same as Mstate-begin, but with the addition of inserting the 
; exception into the environment before calling do-interpret.
(define catch-begin
...
; class-name is the name of the class and env will be the environment
...
; Mvalue evaluates an expression to determine its value.
...
; Mvalue-dot gets the value based off of the specified environment.
; Has different cases for handling the possible operands before the dot.
(define Mvalue-dot
  (lambda (statement env r b c t)
    (cond
...
; This function takes the statement and creats a usable statement for the old Mvalue-funcall
; by parsing and reordering and then getting the correct environment.
...
; Definitions for Mvalue-funcall
...
;
; Mvalue-funcall-with-env executes a function and return the value produced by its return statement.
...
; getFunctionDeclarationEnvironment returns an environment with all bindings within the function's scope. 
; - i.e., all bindings available in the layer it was declared and above. 
; Does not prepend an empty local scope. Based on static scoping.
; Return looks like:
...
; and the environment from which the function was called, bindParameters locates the function closure
; in env and binds the actual parameters to the function's formal parameters.
; Return looks like:
...
; getParamsFromEnvironment returns the list of formal parameters as stored in the function closure in the environment.
(define getParamsFromEnvironment
  (lambda (funName env)
    (car (lookup funName env))))

; bindActualToFormal recursively binds the actual parameters to the formal parameters.
...
; Mbool evaluates a statement for a truth value of true or false.
(define Mbool
...
; lookup determines which layer of the state a variable is in, and calls lookupVal there.
(define lookup
  (lambda (var state)
    (cond
...
; lookupVal gets the specified variable's value from the state.
(define lookupVal
  (lambda (var state)
    (cond
...
; replace-var finds where a variable is in the state to have its value replaced and calls get_replaced.
(define replace_var
...
; get_replaced replaces the value of a variable in the state with the new value specified.
(define get_replaced
  (lambda (var value state)
    (cond
...
; insert inerts a variable into the state. If the variable already exists it replaces it.
...
; env-contains-symbol? checks if the variable is in the current layer of the state.
...
; Definition for state contains
(define var1 car)

; addLevelOfScope adds a level of scope to the given state.
...
; Get the other variables in the current level of the state.
(define resOfVariablesInState cdr)

; Remove the outer most level of scope.
(define getInnerScope cdr)

; Gets the code inside the braces - used with blocks.
...
; secondLevelVariables gets the variables in the next-outer-most scope.
(define secondLevelVariables caadr)

; secondLevelValues gets the values in the next-outer-most scope.
